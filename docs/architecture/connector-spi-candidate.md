...
Claude와 Codex 두 구현에서 확인된 공통 최소 흐름만 후보로 남긴다.
...
`ClaudeHistoryApi`와 `CodexThreadApi`는 서로 다른 내부 adapter이며 합치지 않는다.

## 공통 후보 계약

| 후보 | 최소 의미 | 확인 근거 |
| --- | --- | --- |
| Discovery | 설치, data root, active version, capability별 Availability/Reason | 두 Connector 실제 구현 |
| Source summary | Vendor stable key와 source-modified marker를 제공할 수 있음 | Claude session, Codex thread |
...
- 세 번째 Connector가 이 최소 계약으로 구현되어 과적합 여부를 드러낸 사례
...
Phase 8 완료는 후보 도출까지다. 공개 plugin SPI나 임의 Connector 자동 실행을 의미하지 않는다.
